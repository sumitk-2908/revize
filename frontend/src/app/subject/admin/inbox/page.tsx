    ...
    if (pending) {
      ...
      if (hashes.length > 0) {
        ...
        const approvedByHash = new Map(
          (approved || []).map((doc) => [doc.file_sha256, doc] as const)
        );
        const matches = pending.reduce<Record<number, DocumentWithAnalytics>>((result, doc) => {
          const approvedDocument = doc.file_sha256
            ? approvedByHash.get(doc.file_sha256)
            : undefined;
          if (approvedDocument) result[doc.id] = approvedDocument;
          return result;
        }, {});
        setApprovedDuplicateMatches(matches);
      } else {
        setApprovedDuplicateMatches({});
      }
    } else {
      setPendingDocs([]);
      setApprovedDuplicateMatches({});
            ...
            return (
              <article key={doc.id} className="flex flex-col rounded-2xl border border-warning/20 bg-surface p-4 shadow-sm">
                ...
                <div className="mt-2 flex flex-col gap-2">
                  {approvedDuplicateMatches[doc.id] && (
                    <div className="rounded-lg border border-primary/20 bg-primary/5 p-2 text-xs text-primary">
                      <strong className="font-bold">Possible duplicate:</strong>{" "}
                      <Link
                        href={documentHref(approvedDuplicateMatches[doc.id])}
                        className="font-bold underline underline-offset-2 hover:opacity-80"
                      >
                        View approved original
                      </Link>
                    </div>
                  )}
